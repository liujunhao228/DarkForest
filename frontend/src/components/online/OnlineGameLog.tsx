      ...
      <ScrollArea className="h-32" ref={scrollRef}>
        <div className="p-2 space-y-0.5">
          {recentLogs.map((log: LogEntry) => (
            <div key={log.id} className={`text-[11px] leading-relaxed ${LOG_COLORS[log.type] || 'text-slate-400'}`}>
              {log.type === 'system' ? <span className="font-bold">{'> '}{log.message}</span> : <span>{log.message}</span>}
            </div>
          ))}
        </div>
      </ScrollArea>
