...
// TestInherit_Relic_PrivateReveal 验证继承遗迹时的私有揭示契约。
// 通过 ExecuteLightspeedShip 真实代码路径（随机模式）触发继承：8 名玩家占满星系 1-8，
// 星系 9 为唯一可用跃迁目标且放置了遗迹，因此 p1 必然跃迁至星系 9 并触发继承。
//
// BroadcastOnInherit 不影响继承结果（私有揭示始终设置），仅控制遗留物属性。
func TestInherit_Relic_PrivateReveal(t *testing.T) {
	cases := []struct {
		name               string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// 玩家获得遗迹能量：carry=0 + 8（继承）= 8
			if p1.Energy != 8 {
				t.Errorf("p1 Energy = %d, want 8 (gained relic energy 8)", p1.Energy)
			}
			...
			// 随机模式下公共日志不含遗迹名（位置保密）；私有揭示已通过 LastRelicDiscovery 验证
		})
	}
}
...
func TestLightspeedEscape_BroadcastAttribute(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ExecuteLightspeedShip(state, "p1", 0, "", true, tc.input)
