...
// setupMapHandlerTestDB 打开临时 SQLite 测试库并应用迁移。
func setupMapHandlerTestDB(t *testing.T) *sql.DB {
	t.Helper()
...
// createTestPlayer 创建一个测试玩家并返回其 UUID 字符串。
// role 通常为 "player" 或 "admin"。t.Cleanup 会删除该玩家及其地图。
func createTestPlayer(t *testing.T, pool *sql.DB, queries *db.Queries, role string) (string, *auth.JWTPayload) {
	t.Helper()
	ctx := context.Background()
	pID := uuid.NewString()
	displayName := "p3test-" + pID[:8]
	...
	t.Cleanup(func() {
		...
		_, _ = pool.ExecContext(ctx, "DELETE FROM maps WHERE created_by = ?", pID)
		// 再删玩家（CASCADE 清理 custom_match_queues/custom_match_queue_players）
		_ = queries.DeletePlayer(ctx, pID)
	})
...
func TestCreateMap_AdminOfficial(t *testing.T) {
	...
	if _, err := pool.ExecContext(ctx, "DELETE FROM maps WHERE slug = ?", slug); err != nil {
		t.Fatalf("预清理 slug 失败: %v", err)
	}
	t.Cleanup(func() {
		_, _ = pool.ExecContext(ctx, "DELETE FROM maps WHERE slug = ?", slug)
	})
...
func TestDeleteMap_WaitingRoomBlock(t *testing.T) {
	...
	_, err = pool.ExecContext(ctx,
		...
		queueUUID, "p3test-queue-"+queueUUID[:8], "p3test-waiting-queue", pgUserID, mapID)
	if err != nil {
		t.Fatalf("插入 waiting 队列失败: %v", err)
	}
	t.Cleanup(func() {
		_, _ = pool.ExecContext(ctx, "DELETE FROM custom_match_queues WHERE id = ?", queueUUID)
	})

	// 删除地图应被阻止（409）
	// 注意：httptest.NewRequest 不经过 mux 路由，r.PathValue("id") 返回空串。
	// 用 SetPathValue 显式注入路径参数，模拟路由匹配效果。
	req := newAuthRequest(http.MethodDelete, "/api/maps/"+mapID, "", userPayload)
	req.SetPathValue("id", mapID)
	rec := httptest.NewRecorder()
...
func TestDeleteMap_OwnershipForbidden(t *testing.T) {
	...
	// 注意：httptest.NewRequest 不经过 mux 路由，r.PathValue("id") 返回空串。
	// 用 SetPathValue 显式注入路径参数，模拟路由匹配效果。
	req := newAuthRequest(http.MethodDelete, "/api/maps/"+mapID, "", intruderPayload)
	req.SetPathValue("id", mapID)
	rec := httptest.NewRecorder()
	handler.DeleteMap(rec, req)
