...
func (r *Room) HandleGameAction(playerID string, action string, data json.RawMessage) error {
	...
	case "lightspeedShip":
		...
		//   - Relics 模式（LightspeedOneTime=false）：飞船作为可复用设施，须已部署在 FaceUpCards，由 executeLightspeedShipRelics 校验。
		//   - Classic 模式（LightspeedOneTime=true）：飞船为一次性手牌，由 executeLightspeedShipClassic 内部遍历 player.Hand 查找 ability=="escape" 的卡，无则记录日志返回。
