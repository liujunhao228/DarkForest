根据您提供的详细反馈与澄清，我已整理出一份针对网络桌游开发的完整规则设计文档。文档中修正了数值冲突，明确了打击牌的移动机制、交互逻辑，并剔除了暂不实现的谈判与进阶规则。
...
2.  **摸牌阶段**：从牌堆摸取 4 张牌。
3.  **行动阶段**：玩家从以下两个行动中选择一个执行：
    *   **换牌**：弃任意张手牌，并从牌堆补充相同数量的牌，结束回合。
    *   **打牌**：打出任意张手牌，并从牌堆补充相同数量的牌，结束回合。
...
cards:
  ...
  - id: facility_lightspeed_ship
    ...
    energy: 10
    quantity: 2
    description: 携带能量，跃迁至随机无文明星系（重新抽取星球牌）使用后弃置此牌，处理原有建设牌
