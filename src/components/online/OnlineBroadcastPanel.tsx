          ...
          <div className="bg-slate-900/95 backdrop-blur-sm border border-emerald-900/50 rounded-xl shadow-2xl shadow-emerald-900/20 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-900/30 to-cyan-900/30 px-4 py-3 border-b border-emerald-900/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-400">
                  ...
                  <span className="font-bold text-sm">收到广播信号</span>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
            ...
            <div className="p-4 space-y-4">
              <div className="text-sm text-slate-300">
                ...
                向星系{' '}
                <Badge variant="outline" className="text-cyan-400 border-cyan-400/50 text-xs mx-1">
                  {broadcast.targetSystem}
                </Badge>
                发送了广播信号
                {humanResponse.mustRespond && (
                  <div className="text-red-400 font-bold mt-1 flex items-center gap-1">
              ...
              <div className="bg-slate-800/50 rounded-lg p-3 space-y-2">
                <div className="text-xs text-slate-400 flex items-center gap-1">
                  ...
                  广播范围: {broadcast.range} 格
                </div>
                <div className="text-xs text-slate-400 flex items-center gap-1">
                  <Radio className="w-3 h-3" />
                  类型: {broadcast.subtype === 'cooperation' ? '合作' : '伪装'}
                </div>
              </div>
              ...
              <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {broadcastCards.map((card: Card) => (
                        <div
                          key={card.uid}
                          className="flex-shrink-0 cursor-pointer transition-transform duration-200 hover:scale-105 hover:ring-2 hover:ring-emerald-500/50 rounded-lg"
                          onClick={() => handleRespond(true, card.uid)}
                          role="button"
                          aria-label={`使用 ${card.name} 回应广播`}
                          tabIndex={0}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                              e.preventDefault();
                              handleRespond(true, card.uid);
                            }
                          }}
                        >
                ...
                {!humanResponse.mustRespond && (
                  <Button
                    variant="ghost"
                    className="w-full text-slate-400 hover:text-slate-300 hover:bg-slate-800/50 border border-slate-700/50"
              ...
              <div className="bg-gradient-to-r from-amber-900/30 to-orange-900/30 px-4 py-3 border-b border-amber-900/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-amber-400">
                    <Radio className="w-4 h-4" />
                    <span className="font-bold text-sm">等待回应</span>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            <div className="bg-slate-900/95 backdrop-blur-sm border border-emerald-900/50 rounded-xl shadow-2xl shadow-emerald-900/20 overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-900/30 to-cyan-900/30 px-4 py-3 border-b border-emerald-900/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-emerald-400">
                    <Radio className="w-4 h-4" />
                    <span className="font-bold text-sm">已选择回应者</span>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
              ...
              <div className="px-4 py-3 border-b border-slate-700/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-slate-400">
                    <Radio className="w-4 h-4" />
                    <span className="font-bold text-sm">无人回应</span>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
          ...
          <div className="bg-slate-900/95 backdrop-blur-sm border border-emerald-900/50 rounded-xl shadow-2xl shadow-emerald-900/20 overflow-hidden">
            <div className="bg-gradient-to-r from-emerald-900/30 to-cyan-900/30 px-4 py-3 border-b border-emerald-900/30">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-400">
                  <Radio className="w-4 h-4" />
                  <span className="font-bold text-sm">选择回应者</span>
                </div>
                <Button
                  variant="ghost"
                  size="sm"
              ...
              {respondedPlayers.map((r: BroadcastResponse) => {
                ...
                return (
                  <Button
                    ...
                    onClick={() => {
                      sendAction('selectResponder', { responderId: r.playerId });
                      onClose();
                    }}
                  >
